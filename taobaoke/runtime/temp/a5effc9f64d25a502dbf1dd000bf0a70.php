<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:41:"./application/admin\view\goods_index.html";i:1511577645;s:42:"./application/admin\view\index_header.html";i:1493194503;s:42:"./application/admin\view\index_footer.html";i:1529044280;}*/ ?>
<!DOCTYPE html>
<html>
<head>  
<meta charset="utf-8">
...
<div class="fly-panel fly-panel-user">
  <div class="tpt-admin">
    <div class="tpt-btn" style="float: left;margin: 0 20px 0 0">
      <a href="<?php echo url('goods/add'); ?>" class="layui-btn" style="float: left;margin-right: 5px;"><i class="layui-icon">&#xe608;</i> 添加商品</a>
...
layui.use(['form', 'laydate'], function(){
  ...
  form.on('submit(delete_all)', function(data){
    layer.confirm('确定删除所有商品?', function(index){
      ...
      jq.post('<?php echo url("goods/dels_all"); ?>',function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
  ...
  form.on('submit(delete_all_category)', function(data){
    layer.confirm('<div class="layui-form-item" style="width: 300px;margin-top: 20px;"><label class="layui-form-label">入库分类：</label><div class="layui-input-block"><select id="layui-cid" name="cid" style="width: 150px;padding: 9px 15px;"><?php if(is_array($CategoryList) || $CategoryList instanceof \think\Collection || $CategoryList instanceof \think\Paginator): $i = 0; $__LIST__ = $CategoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($vo['open'] == 1): ?><option value="<?php echo $vo['id']; ?>"><?php echo $vo['name']; ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?></select></div></div>', function(index){
      var cid = jq("#layui-cid").val();
      loading = layer.load(2, {
        shade: [0.2,'#000']
      });
      jq.post('<?php echo url("goods/dels_category"); ?>',{cid:cid},function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
  ...
  form.on('submit(delete_all_time)', function(data){
    layer.confirm('<div class="layui-form-item"><div class="layui-inline"><label class="layui-form-label">开始时间：</label><div class="layui-input-inline"><input type="text" name="startTime" id="startTime" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" onclick="layui.laydate({elem: this})"></div></div><div class="layui-inline"><label class="layui-form-label">结束时间：</label><div class="layui-input-inline"><input type="text" name="endTime" id="endTime" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" onclick="layui.laydate({elem: this})"></div></div></div>', function(index){
      ...
      jq.post('<?php echo url("goods/dels_time"); ?>',{startTime:startTime,endTime:endTime},function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();
